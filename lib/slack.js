...
export async function sendSlackNotification(event, payload) {
  ...
  if (event === 'webhook.test') {
    return webhook.send({
      text: `${EVENT_CONFIGS['webhook.test'].emoji} ${EVENT_CONFIGS['webhook.test'].title}`,
      attachments: [{
        color: EVENT_CONFIGS['webhook.test'].color,
        title: EVENT_CONFIGS['webhook.test'].title,
        text: 'PlanetScale Webhookのテスト通知を受信しました。',
        fields: [
          {
            title: 'Status',
            value: '✅ 接続成功',
          ...
          {
            title: 'Timestamp',
            value: new Date(payload.timestamp * 1000).toLocaleString('ja-JP'),
            short: true
          }
        ],
        footer: 'PlanetScale Integration',
